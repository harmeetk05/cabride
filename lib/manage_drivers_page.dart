...
class _ManageDriversPageState extends State<ManageDriversPage> {
  ...
  Future<void> deleteDriver(String uid) async {
    bool confirm = await _showConfirmDialog("Delete", "Are you sure you want to delete this driver? This cannot be undone.");
    if (confirm) {
  ...
  Future<void> toggleStatus(String uid, bool currentStatus) async {
    ...
    bool confirm = await _showConfirmDialog(action, "Do you want to $action this driver?");
    if (confirm) {
      await FirebaseFirestore.instance.collection('drivers').doc(uid).update({'active': !currentStatus});
    }
  }
    ...
    return await showDialog(
          ...
          builder: (ctx) => AlertDialog(
            ...
            actions: [
              ...
              TextButton(onPressed: () => Navigator.pop(ctx, true), child: Text(title, style: const TextStyle(color: Colors.red))),
            ],
          ),
        ) ??
        false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("Manage Drivers", style: TextStyle(fontWeight: FontWeight.bold)),
        centerTitle: true,
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              String? imageUrl = driver.data().containsKey('imageUrl') ? driver['imageUrl'] : null;return Card(
                ...
                child: ExpansionTile(
                  ...
                  subtitle: Text(isActive ? "● Online" : "○ Suspended", style: TextStyle(color: isActive ? Colors.green : Colors.red, fontSize: 12)),
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: Column(
                              children: [
                                docRow("License", driver['documents']['licenseNumber']),
                                docRow("Aadhaar", "** ** ${driver['documents']['aadhaarLast4']}"),
                              ],
                            ),
                          ),
                          ...
                          FutureBuilder<DocumentSnapshot>(
                            future: FirebaseFirestore.instance.collection('vehicles').doc(driverId).get(),
                            builder: (context, vSnap) {
                              if (!vSnap.hasData || !vSnap.data!.exists) return const Text("No Vehicle Info");
                              var v = vSnap.data!;
                              return Text("🚗 ${v['model']} | ${v['number']} | ${v['capacity']}", 
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              _actionBtn(
                                ...
                                color: Colors.red,onTap: () => deleteDriver(driverId),
                              ),
                            ],
                          )
                        ],
